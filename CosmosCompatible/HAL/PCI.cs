...
namespace Cosmos.HAL
{
    ...
    internal class PCI
    {
        ...
        {
            var BEOS_dev = BEOS.PCI.GetDevice((ushort)vendor, (ushort)device);
            if (BEOS_dev == null) return null;

            PCIDevice dev = new PCIDevice();
            dev.Bus = BEOS_dev.Bus;
            dev.Slot = BEOS_dev.Slot;
            dev.Func = BEOS_dev.Function;
            dev.BaseAddressBar = new List<PCIBaseAddressBar>();
            dev.BaseAddressBar.Add(new(BEOS_dev.Bar0));
            dev.BaseAddressBar.Add(new(BEOS_dev.Bar1));
            dev.BaseAddressBar.Add(new(BEOS_dev.Bar2));
            dev.BaseAddressBar.Add(new(BEOS_dev.Bar3));
            dev.BaseAddressBar.Add(new(BEOS_dev.Bar4));
            dev.BaseAddressBar.Add(new(BEOS_dev.Bar5));
            return dev;
        }
    }
}
