...
namespace BEOS.Drawing.UI
{
    ...
    public class DataGrid : DisplayObject
    {
        ...
        private Bitmap _DataGridIcon_Asc = ResourceHelper.GetBitmap(Resources.DataGridIcon_Asc);
        private Bitmap _DataGridIcon_Desc = ResourceHelper.GetBitmap(Resources.DataGridIcon_Desc);
        ...
        private void RenderHeaders()
        {
            ...
            for (int j = 0; j < _columns.Count; j++)
            {
                ...
                // Draw text
                //0
                _headers.DrawTextInRect(dataGridColumn.Label, x + 5, (_headers.GetBitmap().Height - Font.Height) / 2, dataGridColumn.Width, _headers.GetBitmap().Height, HeadersFontColor, Font);
                ...
                if (j == _selectedDataGridColumnIndex)
                {
                    ...
                    var size = Font.MeasureString(dataGridColumn.Label); //Glide.screen.MeasureString(dataGridColumn.Label, Font);
                    width = (int)size;//.Width;
                    height = (int)Font.Height;//size.Height;
        ...
        private void RenderItem(int index, object[] data, Color backColor, Color fontColor)
        {
            ...
            for (int i = 0; i < _columns.Count; i++)
            {
                ...
                _items.DrawTextInRect(data[i].ToString(), x + 5, y + (RowHeight - Font.Height) / 2, dataGridColumn.Width, RowHeight, fontColor, Font);
        ...
        public override void Render()
        {
            ...
            if (ShowScrollbar)
            {
                ...
                if (_rows.Count > 0)
                    _scrollbarTick = (int)System.Math.Round((double)(travel / _rows.Count),0);
                else
                    _scrollbarTick = 0;
        ...
        public void AddItemAt(int index, DataGridItem dataGridItem)
        {
            if (dataGridItem.Data.Length != _columns.Count)
                return;
                //throw new Exception("The DataGridRow data length does not match the DataGrid's column count.", "dataGridRow");
        ...
        public void SetRowData(int index, object[] data)
        {
            if (RowIndexIsValid(index))
            {
                ...
                {
                    Console.WriteLine("The data length does not match the DataGrid's column count.");
                    return;
                }
        ...
        /// </summary>
        public IFont Font = ResourceHelper.GetFont(12); //Resources.FontResources.droid_reg12);
