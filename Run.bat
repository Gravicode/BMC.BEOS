...
if "%2" equ "qemu" (
	...
	if not exist "C:\Program Files\qemu\qemu-system-x86_64.exe" (
		echo "Please install QEMU in order to debug BEOS!(do not modify the path) https://www.qemu.org/download/#windows"
		pause
		exit
	)
	...
	if exist "C:\Program Files (x86)\VMware\VMware Workstation\vmplayer.exe" (
		"C:\Program Files (x86)\VMware\VMware Workstation\vmplayer.exe" "Tools\VMWare\BEOS\BEOS.vmx"
	) else if exist "C:\Program Files (x86)\VMware\VMware Player\vmplayer.exe" (
		"C:\Program Files (x86)\VMware\VMware Player\vmplayer.exe" "Tools\VMWare\BEOS\BEOS.vmx"
	) else (
		echo "Please install VMWare Player in order to Run BEOS!"
		pause
		exit
	)
) else (
